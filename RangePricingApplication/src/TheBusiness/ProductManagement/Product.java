...
public class Product {
    ...
    public int getTopOrderPriceHigherTarget() {
        int currenthigherorderprice = 0;
        for (OrderItem oi : orderitems) {
            if (currenthigherorderprice == 0 && oi.getActualPrice() > this.targetPrice) {
                currenthigherorderprice = oi.getActualPrice(); // initial step 
            } else if (oi.getActualPrice() > currenthigherorderprice && oi.getActualPrice() > this.targetPrice) {
                currenthigherorderprice = oi.getActualPrice(); //we have a new higher OrderPrice
            }
                
        }
        return currenthigherorderprice;
    }
    
    public int getTopOrderPriceLowerTarget() {
        int currentlowerorderprice = 0;
        for (OrderItem oi : orderitems) {
            if (currentlowerorderprice == 0 && oi.getActualPrice() < this.targetPrice) {
                currentlowerorderprice = oi.getActualPrice(); // initial step 
            } else if (oi.getActualPrice() < currentlowerorderprice && oi.getActualPrice() < this.targetPrice) {
                currentlowerorderprice = oi.getActualPrice(); //we have a new lower OrderPrice
            }
                
        }
        return currentlowerorderprice;
    }
